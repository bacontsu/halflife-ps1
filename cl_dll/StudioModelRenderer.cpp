...
void CStudioModelRenderer::StudioDrawBlobShadow()
{
	...
	Vector camNormal;
	VectorSubtract(camPos, hitCenter, camNormal);
	camNormal = camNormal.Normalize();

	for (int j = 0; j < 3; ++j)
	{
		hitCenter[j] = hitCenter[j] + camNormal[j] * 2.0f;
	}
	...
	for (int i = 0; i < NUM_SEGMENTS; ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			ring[i][j] = ring[i][j] + camNormal[j] * 2.0f;
		}
	...
	// Save the existing shade mode because GL_SMOOTH is not covered by
	// GL_ENABLE_BIT / GL_COLOR_BUFFER_BIT.
	GLint oldShadeModel = GL_FLAT;
	...
	for (int i = 0; i <= NUM_SEGMENTS; ++i)
	{
		...
		// Soft edge, but never completely invisible.
		glColor4f(0.0f, 0.0f, 0.0f, edgeAlpha);
		glVertex3fv(ring[index]);
	}

	glEnd();

	// Restore the renderer's previous shade mode.
